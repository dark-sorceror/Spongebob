...
class Giveaway extends EventEmitter {
    ...
    get gotoGiveaway() {
        const button = new Discord.MessageButton().setLabel('Jump to Giveaway').setStyle('LINK').setURL(this.messageURL);
        return button;
    }

    get endgiveawaybutton() {
        const button = new Discord.MessageButton().setLabel('End').setStyle('SECONDARY');
    ...
    get data() {
        return {
            ...
            requirements:
                !this.options.requirements || typeof this.options.requirements === 'string'
                    ? this.options.requirements || undefined
                    : serialize(this.options.requirements),
     ...
     */
        ...
        return new Promise(async (resolve, reject) => {
            ...
            let theguild = this.client.guilds.cache.get(this.guild)

            const hwin = new Discord.MessageEmbed()
                .setTitle("Your Giveaway Ended!")
                .setColor("#2F3136")
                .setDescription(`Your giveaway for **[${this.prize}](${this.messageURL})** in **${this.guildId}** has finished.\nYou have **${this.winnerCount}** winner(s).`)
            const mwin = new Discord.MessageEmbed()
                .setTitle("You Won!")
                .setColor("GREEN")
                .setDescription(`Lucky! You were chosen amongst **${this.winnerCount}** user(s) to win **[${this.prize}](${this.messageURL})** giveaway in **${this.guildId}**!\nPlease wait patiently while the host gives your prize.`)
            const thehost = this.client.users.cache.get(this.hostedBy.slice(2,-1))
            const row = new Discord.MessageActionRow()
			.addComponents(
				this.gotoGiveaway
			);
            winners.forEach((member) => {
                member.send({ embeds: [mwin], components: [row] });
            });
            thehost.send({ embeds: [hwin], components: [row] })
                ...
                if (message?.length > 2000) {
                    ...
                    if (firstContentPart.length) {
                        channel.send({
                            ...
                            components: [this.manager.gotoGiveaway]
                        });
                    }
