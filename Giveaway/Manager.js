...
const { validateEmbedColor } = require('./utils');
...
class GiveawaysManager extends EventEmitter {
    ...
    generateMainEmbed(giveaway, lastChanceEnabled = false) {
        const embed = new Discord.MessageEmbed()
            ...
            .setFooter({
                text: `${giveaway.winnerCount} winner(s) | Ends at `
            })
            .setDescription(
                `> React with :tada: to enter\n> Ends **<t:${parseInt(giveaway.endAt/ 1000)}:R>** (<t:${parseInt(giveaway.endAt/1000)}:f>)\n> Hosted by ${giveaway.hostedBy}`
            )
    ...
    generateEndEmbed(giveaway, winners) {
        ...
        return new Discord.MessageEmbed()
            ...
            .setFooter({ text: `${giveaway.winnerCount} winner(s) | Ended at ` })
            
            .setDescription(`> Winner(s): ${formattedWinners}\n> Hosted by: ${giveaway.hostedBy}`)
    ...
    generateNoValidParticipantsEndEmbed(giveaway) {
        ...
        .setColor(giveaway.embedColorEnd)
        .setFooter({ text: `${giveaway.winnerCount} winner(s) | Ended at ` })
        .setDescription(`> Winner(s): No one!\n> Hosted by: ${giveaway.hostedBy}`)
        .setTimestamp(giveaway.endAt)
        return giveaway.fillInEmbed(embed);
    }
	...
	async endByButton(giveaway) {
		...
		if (res == 'giveawayEnded') button.reply({ content: "Giveaway already", ephemeral: true });
	}
    */
    /**
    ...
    start(channel, options) {
        return new Promise(async (resolve, reject) => {
            ...
            const giveaway = new Giveaway(this, {
                startAt: Date.now(),
                endAt: options.isDrop ? null : Date.now() + options.duration,
...
GiveawaysManager.on('giveawayEnded', (giveaway, winners) => {
    ...
    hwin = new Discord.MessageEmbed()
    .setTitle("Your Giveaway Ended!")
    .setColor("#2F3136")
    .setDescription(`Your giveaway for **(${giveaway.prize})[${giveaway.messageURL}]** in **${giveaway.guild}** has finished.\nYou have **${giveaway.winnerCount}** winner(s).`)
    mwin = new Discord.MessageEmbed()
    .setTitle("You Won!")
    .setColor("GREEN")
    .setDescription(`Lucky! You were chosen amongst **${giveaway.winnerCount}** user(s) to win **(${giveaway.prize})[${giveaway.messageURL}]** giveaway in **${giveaway.guild}**!\nPlease wait patiently while the host gives your prize.`)
    winners.forEach((member) => {
        member.send({embeds: [mwin]});
    });
    hwin.send({embeds:[hwin]})
